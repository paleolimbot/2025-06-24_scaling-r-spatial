...
dbExecute(
  con,
  glue::glue("CREATE TABLE IF NOT EXISTS lakes AS
...
## Driving spatial databases: ADBC + PostGIS {background-image="bg/wb3.png"}
...
```{r}
con |>
  execute_adbc("CREATE TABLE IF NOT EXISTS lakes (objectid float8, geometry GEOMETRY)")
lakes |>
  as_tibble() |>
...
```{r}
con |>
  execute_adbc("CREATE TABLE IF NOT EXISTS elevation (geometry GEOMETRY)")
